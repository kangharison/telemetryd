...
## 10. 아키텍처 리팩토링 — 플랫폼 + 서비스 계층

### 10.1 무엇이 문제였나

기능이 늘면서 두 개의 God 구조가 생겼다:

- **God Interface**: `Backend` Protocol에 메서드 17개가 5개 도메인(큐/성능/
  이벤트/토폴로지/프로파일러)에 걸쳐 있었다. 성능만 필요한 소비자도 큐/트리/
  프로파일러 메서드를 다 가진 객체를 받아야 했고, mock은 실제로 안 쓰는
  메서드까지 전부 따라 구현해야 했다.
- **God Object**: `DrgnBackend` 767줄 / `MockBackend` 753줄이 그 17개를 전부
  구현했다. 한 도메인을 고치려면 이 거대한 파일을 열어야 했고, 새 기능 하나에
  models/proto/convert/server/client/base/drgn/mock/cli/web 10개 파일이 함께 움직였다.

### 10.2 새 구조

```
platform/          도메인 지식 없는 기반 설비 (Ports & Adapters)
  kernel/          drgn Program 생명주기 + 접속 방식(QMP/로컬/심볼)
  ebpf/            수집기 로그 접근(전체/꼬리/증분 커서)
  cache.py         AdaptiveTtlCache — TTL을 직전 실측 소요시간에 비례(§9.16)

services/<이름>/   도메인별 수직 슬라이스
  contract.py      좁은 인터페이스(Protocol)
  service.py       플랫폼 위 실제 구현
  mock.py          커널/수집기 없이 도는 구현
  registry.py      조립 지점 — 이름 -> 구현

composition.py     조립 루트. 무엇을 무엇으로 채울지 아는 유일한 곳
```

서비스별 의존이 곧 배포 단위다:

| 서비스 | platform.kernel | platform.ebpf |
|---|:--:|:--:|
| queues | O | — |
| topology | O | — |
| perf | — | O |
...
perf/events는 drgn/QMP 권한이 아예 없는 프로세스로 뗄 수 있고, queues/topology는
수집기가 안 떠 있어도 완전히 동작한다. 이건 §6에서 정한 역할 분담(eBPF=상시
카운터, drgn=온디맨드 스냅샷)이 의존성 그래프로 드러난 것이다.

### 10.3 모놀리스와 마이크로서비스가 갈리는 지점

```python
# 모놀리스(지금)
registry.register("perf", EbpfPerfService(log), PerfService)
# 마이크로서비스(구조만 준비)
registry.register("perf", RemotePerfClient(addr), PerfService)
```

둘 다 같은 계약을 만족하므로 **호출 측 코드는 한 줄도 안 바뀐다**(Strategy).
요구대로 지금은 구조만 갖추고 모놀리스 1프로세스로 구동한다. 마이크로서비스로
가려면 `composition.py`에 `build_remote()`를 추가하는 것으로 끝이고, 서비스와
어댑터는 그대로다.

쪼갤 때 실제로 걸림돌이 되는 건 코드 구조가 아니라 **QMP가 클라이언트를 하나만
받는다는 제약**(§9.2)이다 — drgn을 쓰는 서비스(queues/topology/profiler)를 서로
다른 프로세스로 띄우려면 커널 접근을 중계하는 프로세스가 하나 필요하다. 그래서
지금 구조에서도 커널 세션은 조립 루트가 **하나만** 만들어 공유시킨다.

### 10.4 규칙을 문서가 아니라 테스트로 강제한다

리팩토링의 진짜 위험은 "지금 깔끔한가"가 아니라 "6개월 뒤에도 깔끔한가"다.
계층 규칙은 문서에 적어두면 바쁠 때 import 한 줄로 조용히 무너진다. 그래서
`tests/test_architecture.py`가 소스의 import를 AST로 읽어 기계적으로 막는다:

- platform은 services/전송계층을 모른다 (역방향 의존 금지)
- **services끼리 직접 import 금지** — 이게 깨지면 마이크로서비스로 못 쪼갠다.
  다른 서비스가 필요하면 계약이나 콜러블을 주입받는다(토폴로지가 장치 조회를
  받는 방식이 그 예)
- services는 gRPC/HTTP를 모른다 (전송을 바꿔도 서비스는 안 바뀐다)
- 플랫폼 모듈 이름에 도메인 어휘(nvme/queue/perf…)가 섞이지 않는다
- **가드 자체가 위반을 실제로 잡는지도 검증**한다 — 빈 디렉터리를 훑고 있어서
  통과하는 검사는 없느니만 못하다

`tests/test_services.py`는 조립 불변식을 고정한다. 그중 하나가 "큐/토폴로지/
프로파일러가 **같은 커널 세션을 공유**한다"인데, 이걸 어기면 캐시가 서비스마다
따로 생겨 §9.16에서 대시보드를 멈춘 문제가 그대로 재현된다.

### 10.5 결과

| | 이전 | 이후 |
|---|---:|---:|
| DrgnBackend | 767줄 | **218줄**(위임만 남은 파사드) |
| MockBackend | 753줄 | 530줄 |
| 테스트 | 128개 | **148개** |

`Backend` 파사드는 남겨뒀다(strangler) — 기존 gRPC/CLI/웹 호출부를 안 깨뜨린
채 로직만 이사시켰기 때문이다. 다음 단계로 호출부를 레지스트리에 직접 붙이면
이 파사드는 걷어낼 수 있다.

**라이브 검증이 잡은 버그 1개**: 큐 서비스로 옮긴 코드가 `self.kind`
(DrgnBackend의 클래스 속성)를 참조하고 있었다. 단위 테스트는 이 경로를
MockBackend로만 타서 못 잡았다 — drgn 없는 환경의 구조적 커버리지 공백이다.
매 단계 라이브 게스트로 확인한 덕에 드러났다. 리팩토링에서 "테스트가 통과하니
안전하다"가 성립하지 않는 영역이 있다는 실례.
